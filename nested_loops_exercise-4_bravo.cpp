...
int main() {
	...
	do {
		...
		cout	<< " ### exercise 4 - right-angled triangle no.2 ### "
				<< "\n enter a number: ";
		cin		>>			inputNumber;
...
/*	using C++ compiler from GCC via console
	...
	compile for debug:
g++ -Og -pedantic-errors -Wall -Wextra nested_loops_exercise-4_bravo.cpp -o exe-nested_loops_exercise-4_bravo.debug

	clear console, compile as final executable, and run program:
clear && g++ -O3 nested_loops_exercise-4_bravo.cpp -o exe-nested_loops_exercise-4_bravo.release && ./exe-nested_loops_exercise-4_bravo.release
